...
* Existing Watches with the same `name` as the incoming Watches will be
  overridden by the new config.
...
The codebase can be found at https://github.com/mishina2228/resurrected_god. To get started,
...
We recommend using link:https://github.com/rbenv/rbenv[rbenv] and
link:https://github.com/rbenv/ruby-build[ruby-build] to manage multiple
versions of Ruby and their separate gemsets. Any changes to god must work on
Ruby 2.6+.

God uses link:https://bundler.io[bundler] to deal with development
...
to the documentation. It can be found in the `doc` directory. The
documentation is done in link:https://asciidoc.org[AsciiDoc] format
